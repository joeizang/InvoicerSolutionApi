...
public static class PlatformServiceQueries
{
    ...
    public static readonly Func<InvoicerApiContext, Guid, IAsyncEnumerable<PlatformServiceApiModel>> 
        GetPlatformServices =
            EF.CompileAsyncQuery(
                (InvoicerApiContext context, Guid platformServiceId) =>
                    context.PlatformServices.AsNoTracking()
                    .Where(x => x.Id == platformServiceId)
                    .OrderBy(x => x.Name)
